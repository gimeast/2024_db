...
- 체크 예외는 비즈니스 로직상 의도적으로 던지는 예외에만 사용하자.
  - 아래 처럼 예외를 잡아서 반드시 처리해야 하는 문제일 때는 체크 예외를 사용해야한다.
    - 계좌이체 실패시 예외
    - 결제시 포인트 부족 예외
    - 로그인 ID, PW 불일치 예외
  - 물론 이 경우 100% 체크 예외로 만들어야하는 것은 아니다.  
    다만 계좌 이체 실패처럼 매우 심각한 문제는 개발자가 실수로 예외를 놓치면 안된다고 판단할 수 있다.  
    이경우 체크 예외로 만들어 두면 컴파일러를 통해 놓친 예외를 인지할 수 있다.
...
## 예외 포함과 스택 트레이스
- 예외를 전환할 때는 꼭!!! 기존 예외를 포함해야한다. 그렇지 않으면 스택트레이스를 확인할 때 심각한 문제가 발생한다.

## 정리
```
예외의 계층에는 Object -> Throwable이 있고 하위에 Exception과 Error로 나뉜다.
Exception은 Checked와 Unchecked로 나뉜다.

예외의 기본 규칙은 catch 하거나 throws 하거나 해야한다.

기본적으로 언체크 예외를 사용하자
체크 예외는 반드시 잡아서 처리해야하는 문제인 경우 사용해야한다 -> 커스텀해서 사용하는 예외 extends Exception

런타임 예외는 던지는거를 생략 가능하며 JDBC에서 JPA로 변경을 해도 코드가 변경되지 않는다.
...
서비스가 처리할 수 없는 SQLException에 대한 의존을 제거하려면
repository가 던지는 SQLException 체크예외를 runtimeException으로 전환해서 서비스 계층에 던지면 된다.

인터페이스 도입시 문제점은 구현 클래스에서 체크예외를 던지려면 인터페이스에도 던지는 부분이 선언 되어있어야 한다.
결과적으로 특정 구현 기술에 종속되는 인터페이스를 만들 수 밖에 없다.
```
...
- JDBC나 JPA를 사용할때 발생하는 예외를 스프링이 제공하는 예외로 변환해주는 역할도 스프링이 제공한다.
- 최상위 예외는 DataAccessException이며 RuntimeException을 상속받았다.  
  DataAccessException은 Transient(일시적)과 NonTransient 예외로 나뉜다.
